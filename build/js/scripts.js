...
app.config(function($routeProvider, $locationProvider) {
    ...
    when('/dashboard/cours', {
        title: "Cours",
        templateUrl: 'partials/cours.html',
        controller: 'coursCtrl',
        id: 'cours'
    }).
    when('/edit/user/:pseudo', {
        title: "Modifier un utilisateur",
        templateUrl: 'partials/editUser.html',
        controller: 'editUserCtrl',
        id: 'editUser'
    }).
    when('/edit/cours/:coursId', {
        title: "Modifier un cours",
        templateUrl: 'partials/editCours.html',
        controller: 'editCoursCtrl',
        id: 'editCours'
    }).
    when('/todo', {
        title: "Tâches",
    ...
    when('/todo/history', {
        ...
        templateUrl: 'partials/historyTodo.html',
        controller: 'historyTodoCtrl',
        id: 'historyTodo'
    }).
    when('/404', {
...
app.run(['$rootScope', '$location', 'AuthService', function($rootScope, $location, AuthService) {
    $rootScope.$on('$routeChangeStart', function(event, next, current) {
        if(!AuthService.isAuthenticated()) {
            var allowedRoutes = ['/', '/login'];
            var isAllowed = allowedRoutes.indexOf($location.path()) > -1 ;
...
app.controller('appCtrl', function($scope, AuthService, $location, AUTH_EVENTS, $window, $rootScope) {
...
app.controller('dashboardCtrl', function($scope, $http, API_ENDPOINT, AuthService, $location) {
    $http.get(API_ENDPOINT.url + '/status').then(function(result) {
    ...
    if (!AuthService.isAuthenticated()) {
        ...
        $http.get(API_ENDPOINT.url + '/statistics').then(function(result) {
            $scope.stats = result.data;
        });
    }
});

app.controller('usersCtrl', function($scope, $http, API_ENDPOINT, AuthService, $location) {
    $http.get(API_ENDPOINT.url + '/statistics').then(function(result) {
        $scope.stats = result.data;
        $scope.usersLength = $scope.stats.users.length;
        console.log($scope.usersLength);
    });
    $scope.currentPage = 0;
    $scope.pageSize = 20;
    $scope.usersLength = 0;
    ...
    $scope.momentjs = function(date) {
        return moment(date).locale("fr").format('LLL');
    }
    $scope.getUsers = function(page) {
        $scope.currentPage = page;
        $http.get(API_ENDPOINT.url + '/listUsers', {
            params: {
...
app.controller('coursCtrl', function($scope, $http, API_ENDPOINT, AuthService, $location) {
    $http.get(API_ENDPOINT.url + '/statistics').then(function(result) {
        $scope.stats = result.data;
        $scope.coursLength = $scope.stats.cours.length;
    });
    $scope.currentPage = 0;
    $scope.pageSize = 20;
    $scope.coursLength = 0;
    ...
    $scope.getCours = function(page) {
        $scope.currentPage = page;
        $http.get(API_ENDPOINT.url + '/listCours', {
            params: {
                page: $scope.currentPage
            }
        }).then(function(result) {
...
app.controller('loginCtrl', function($scope, $http, API_ENDPOINT, AuthService, $location) {
    $scope.isAuthenticated = function() {
        return AuthService.isAuthenticated();
    }
    if (AuthService.isAuthenticated())  {
...
app.controller('createTodoCtrl', function($scope, $http, API_ENDPOINT, AuthService, $location) {
    $scope.createTask = function() {
        $http.post(API_ENDPOINT.url + '/createTodo', {
            task: $scope.task
        }).then(function(result) {
...
app.controller('todoCtrl', function($scope, $http, API_ENDPOINT, AuthService, $location) {
    function getTodo() {
        $http.get(API_ENDPOINT.url + '/getTodo', {
            params: {
                completed: false
            }
        }).then(function(result) {
    ...
    $scope.remove = function(_id) {
        $http.delete(API_ENDPOINT.url + '/removeTodo', {
            params: {
                todoId: _id
            }
        }).then(function(result) {
    ...
    $scope.terminer = function(_id) {
        $http.post(API_ENDPOINT.url + '/changeTodoState', {
            todoId: _id,
            completed: "true"
        }).then(function(result) {
            if (result.data.success) {
...
app.controller('historyTodoCtrl', function($scope, $http, API_ENDPOINT, AuthService, $location) {
    function getTodo() {
        $http.get(API_ENDPOINT.url + '/getTodo', {
            params: {
                completed: true
...
app.controller('editUserCtrl', function($scope, $http, API_ENDPOINT, AuthService, $location, $routeParams) {
    var userPseudo = $routeParams.pseudo;
    $http.get(API_ENDPOINT.url + '/getUserInfos', {
        params: {
            pseudo: userPseudo
        }
    }).then(function(result) {
        $scope.profile = result.data;
        $scope.editedUser = angular.copy($scope.profile);
        delete $scope.editedUser._id;
    });

    $scope.saveUser = function() {
        $scope.editedUser._id = $scope.profile._id;
        $http.post(API_ENDPOINT.url + '/saveUserInfos', {
...
.constant('API_ENDPOINT', {
  url: 'https://localhost:8089/admin'
});
...
angular.module('revizone')
    ...
    .service('AuthService', function($q, $http, API_ENDPOINT, $rootScope) {
        var LOCAL_TOKEN_KEY = 'f798df32sf48z97';
        ...
        var getUser = function()  {
            return $q(function(resolve, reject) {
                $http.get(API_ENDPOINT.url.replace('admin', 'api') + '/getuser').then(function(result) {
                    if (result.data) {
                        resolve(result.data);
                    } else {
